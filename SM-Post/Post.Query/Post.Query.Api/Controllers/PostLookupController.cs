...
namespace Post.Query.Api.Controllers
{
    ...
    public class PostLookupController : ControllerBase
    {
        ...
        public async Task<ActionResult> GetPostsWithCommentsAsync()
        {
            try
            {
                var posts = await _queryDispatcher.SendAsync(new FindPostsWithComments());
                return NormalResponse(posts);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> GetPostsWithLikesAsync(int numberOfLikes)
        {
            try
            {
                var posts = await _queryDispatcher.SendAsync(new FindPostsWithLikesQuery { Likes = numberOfLikes });
                return NormalResponse(posts);
            }
            catch (Exception ex)
            {
